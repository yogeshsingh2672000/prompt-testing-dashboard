...
export function TestCasesSection({
  ...
  const handleAIGenerate = async () => {
    ...
    try {
      ...
      const sampleInput =
        testCases.length > 0 ? testCases[0].input : "general input";
      ...
      if (data.testCases) {
        const newCases = (data.testCases as GeneratedTestCasePayload[]).map(
          (tc) => ({
  ...
  return (
    <div className="min-w-0 flex-1 space-y-8">
      <div className="bg-white/40 dark:bg-zinc-900/40 border border-zinc-200 dark:border-zinc-800 p-6 md:p-8 rounded-[2.5rem] backdrop-blur-xl shadow-2xl relative overflow-hidden group transition-all duration-500 min-h-[500px] h-full">
        ...
        <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6 mb-10">
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-2 mt-2">
              <span className="text-sm font-black text-zinc-900 dark:text-zinc-100 italic">
                {t("activeSuite")}
              </span>
          ...
          <div className="w-full xl:w-auto flex flex-col sm:flex-row gap-3">
            ...
            >
              {isGenerating ? (
                ...
                <Sparkles
                  size={16}
                  className="text-teal-500 group-hover:rotate-12 transition-transform"
                />
              )}
        ...
        <div className="space-y-8 h-full overflow-y-auto pr-4 custom-scrollbar pb-8">
